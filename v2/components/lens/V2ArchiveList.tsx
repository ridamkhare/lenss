...
import {
  ...
  listV2Notices,
  type SavedV2Notice,
} from "@/v2/lib/v2Storage"

/**
 * V2 ArchiveList.
 *
 * Reads V1's lens:archive (unmodified) and V2's lens:v2:archive
 * (sidecar), then renders each V1 item with its attached V2 notice
 * if one exists. Items saved on V1 routes show no V2 notice — same
 * as V1's archive.
 *
 * Behavior matches V1's ArchiveList for the V1 portion. The V2
 * notice is appended below the V1 signals when present.
 */
...
export function V2ArchiveList() {
  const router = useRouter()
  const [items, setItems] = useState<SavedItem[] | null>(null)
  const [notices, setNotices] = useState<Record<string, SavedV2Notice>>({})
  const [expanded, setExpanded] = useState<string | null>(null)

  useEffect(() => {
    setItems(listItems())
    setNotices(listV2Notices())
  }, [])
...
function NoticeFooter({ v2 }: { v2: SavedV2Notice }) {
  return (
    <div
